...
class CopyrightRefService
{
    ...
    /**
     * @return Collection<int, StoryRefCopyright>
     */
    public function getAll(): Collection
    {
        $items = $this->cache->remember(
            'storyref:copyrights:public:list',
            self::CACHE_TTL_SECONDS,
            function () {
                return StoryRefCopyright::query()
                    ->orderBy('order')
                    ->orderBy('name')
                    ->get();
            }
    ...
    public function getOneById(int $id): ?StoryRefCopyright
    {
        /** @var StoryRefCopyright|null $found */
        $found = $this->getAll()->firstWhere('id', $id);

        return $found instanceof StoryRefCopyright ? $found : null;
    }

    public function getOneBySlug(string $slug): ?StoryRefCopyright
    {
        ...
        /** @var StoryRefCopyright|null $found */
        $found = $this->getAll()->first(function (StoryRefCopyright $model) use ($normalized) {
     ...
     */
    public function create(array $data): StoryRefCopyright
    {
        $model = new StoryRefCopyright();
        $model->fill($data);
        $model->save();
     ...
     * @param array{name?:string,slug?:string,description?:string|null,is_active?:bool,order?:int|null} $data
     */
    public function update(int $id, array $data): ?StoryRefCopyright
    {
        /** @var StoryRefCopyright|null $model */
        $model = StoryRefCopyright::query()->find($id);
        if (! $model) {
            return null;
        }
    ...
    {
        $this->cache->forget('storyref:copyrights:public:list');
    }
}
