@props(['copyright' => null, 'nextOrder' => null])

@php
    $isEdit = $copyright !== null;
@endphp

<div class="flex flex-col gap-6" x-data="{
    name: '{{ old('name', $copyright?->name ?? '') }}',
    slug: '{{ old('slug', $copyright?->slug ?? '') }}',
    ...
    <div>
        <x-shared::input-label for="name" :required="true">
            {{ __('story_ref::admin.copyrights.form.name') }}
    ...
    <div>
        <x-shared::input-label for="slug" :required="true">
            {{ __('story_ref::admin.copyrights.form.slug') }}
        </x-shared::input-label>
        <x-shared::text-input
            type="text"
            id="slug"
            ...
            pattern="[a-z0-9\-]+"
        />
        <p class="text-xs text-fg/60 mt-1">{{ __('story_ref::admin.copyrights.form.slug_help') }}</p>
    ...
    <div>
        <x-shared::input-label for="description">
            {{ __('story_ref::admin.copyrights.form.description') }}
        </x-shared::input-label>
        <textarea
            id="description"
            name="description"
        ...
        >{{ old('description', $copyright?->description ?? '') }}</textarea>
    ...
    <div>
        <x-shared::toggle
            name="is_active"
            :checked="old('is_active', $copyright?->is_active ?? true)"
            :label="__('story_ref::admin.copyrights.form.is_active')"
        />
    ...
    <div class="flex gap-4">
        <x-shared::button type="submit" color="primary" icon="save">
            {{ $isEdit ? __('story_ref::admin.copyrights.form.update') : __('story_ref::admin.copyrights.form.create') }}
        </x-shared::button>
        <a href="{{ route('story_ref.admin.copyrights.index') }}">
            <x-shared::button type="button" color="secondary">
                {{ __('story_ref::admin.copyrights.form.cancel') }}
            </x-shared::button>
        </a>
    </div>
</div>
