  ...
  <div class="flex gap-4">
    ...
    <div class="flex-1">
      ...
      <div class="flex items-center gap-3">
        ...
        @if(auth()->check() && (int) auth()->id() === (int) $comment->authorId)
      ...
      <div class="mt-3 flex gap-4 text-xs text-gray-500">
        ...
        @if((!$isChild && !$hasChildren) || ($isLastChild??false))
      ...
      @php($replyId = $isChild ? $parentCommentId : $comment->id)
        <div class="mt-3" x-show="activeReplyId === {{ $replyId }}">
          <form method="POST" action="{{ route('comments.store') }}" class="space-y-2">
            ...
            <x-shared::editor id="reply-editor-{{ $replyId }}" name="body" :nbLines="5" class="mt-1 block w-full" placeholder="{{ __('comment::comments.form.body.placeholder') }}" />
            @error('body')
      ...
      @if(!empty($comment->children))
        <ul class="mt-6 space-y-6 border-l pl-6">
          @foreach($comment->children as $child)
            @include('comment::components.partials.comment-item', ['comment' => $child, 'isChild' => true, 'isLastChild' => $loop->last, 'parentCommentId' => $comment->id])
          @endforeach
        </ul>
      @endif
    </div>
