...
class BackfillChapterCommentNotificationsCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        // Step 1: Delete all existing notifications for all comment types (legacy + current)
        $this->info('Deleting existing notifications...');
        $deletedCount = 0;
        $deletedCount += $this->notifications->deleteNotificationsByType(ChapterCommentNotification::type());
        $deletedCount += $this->notifications->deleteNotificationsByType(ChapterRootCommentNotification::type());
        $deletedCount += $this->notifications->deleteNotificationsByType(ChapterReplyCommentNotification::type());
        ...
        $countCurrent = fn() =>
            $this->notifications->countNotificationsByType(ChapterRootCommentNotification::type())
            + $this->notifications->countNotificationsByType(ChapterReplyCommentNotification::type());
        ...
        foreach ($eventDtos as $dto) {
            ...
            $notificationsBefore = $countCurrent();
            ...
            if ($countCurrent() > $notificationsBefore) {
                $created++;
            } else {
                $skipped++;
            }
