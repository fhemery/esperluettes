...
<x-app-layout>
    ...
    <div class="w-full flex flex-col gap-4">
        ...
        <!-- Filters Section -->
        <div class="w-full surface-read text-on-surface flex flex-col gap-4">
                ...
                <form method="GET" action="{{ url('/stories') }}" x-data
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-6 items-start">
                        ...
                        <div class="flex flex-col gap-2">
                            ...
                            <x-shared::select-with-tooltips name="type" :options="$referentials['types'] ?? []" :selected="$currentType ?? ''"
                                :placeholder="__('story::index.filters.type.placeholder')" valueField="slug" labelField="name" descriptionField="description"
                                color="accent" />
                        </div>
                        ...
                        <div>
                            ...
                            <div class="mt-2">
                                <x-shared::searchable-multi-select name="audiences[]" :options="$referentials['audiences'] ?? []"
                                    :selected="$currentAud" :empty-text="__('story::shared.no_results')" :placeholder="__('story::index.filters.audiences.placeholder')" color="accent" />
                            </div>
                        </div>

                        <!-- Trigger warnings -->
                        <div>
                            <label class="flex gap-2 text-sm font-medium text-fg">
                            ...
                            <div class="mt-2">
                                <x-shared::searchable-multi-select name="exclude_tw[]" :options="$referentials['trigger_warnings'] ?? []"
                                    :selected="$currentExTw" :empty-text="__('story::shared.no_results')" :placeholder="__('story::index.filters.trigger_warnings.placeholder')" color="accent" />
                            </div>
                            <div class="mt-4">
                                <label class="inline-flex flex-wrap items-center gap-2 text-sm text-fg">
                                    <input type="checkbox" name="no_tw_only" value="1"
                                        {{ !empty($currentNoTwOnly) ? 'checked' : '' }}
                        ...
                        <div class="col-span-full flex gap-2 items-center justify-center">
                            <x-shared::button color="accent"
                                type="submit">{{ __('story::index.filter') }}</x-shared::button>
                            <a href="{{ url('/stories') }}" class="ml-3 text-sm text-gray-600 hover:text-gray-900">
                                <x-shared::button color="neutral" :outline="true"
                                    type="button">{{ __('story::index.reset_filters') }}</x-shared::button>
                            </a>
                        </div>
                    </div>
                </form>
