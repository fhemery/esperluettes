...
**`StoryPreferenceService` is a container singleton and memoizes for the whole request.** It is bound in `StoryServiceProvider::register()`; that binding is what makes the memo useful, since Blade resolves it with `app(...)` once per card. The consequence for tests: the container is not rebuilt between two HTTP calls inside one test, so a test that flips one of the two preferences *between* two requests must call `app()->forgetInstance(StoryPreferenceService::class)` afterwards, or split into two test cases. Setting the preference before the first request needs nothing.
...
- **SettingsPublicApi** (`Settings` domain) — registers the `hide-comments-section` boolean (default `false`) into *Profile's* settings tab and privacy section, inside `app->booted()` so those exist first. Story owns it because it gates Story's comments tab; `ProfileCommentsPolicy` is the only reader. The stored `domain` column is `profile` (the settings tab id), not `story`.
  Story also owns its **own** tab `stories` (`TAB_STORIES`, order 15) with the section `reading` (`SECTION_READING`) and two booleans defaulting to `false`: `hide-trigger-warnings` (`KEY_HIDE_TRIGGER_WARNINGS`) and `hide-stories-with-tw` (`KEY_HIDE_STORIES_WITH_TW`). Read them through `StoryPreferenceService`, never through `SettingsPublicApi` directly — see the invariant below. Both registrations run from the same `app->booted()` closure but from **separate** methods: `registerSettings()` returns early once its own parameter exists, so appending to it would silently skip on a second boot.
