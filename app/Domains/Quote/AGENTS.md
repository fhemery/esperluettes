...
- **`canQuote` takes a story ID, not a chapter ID**, and blocks authors (including co-authors — same `author` role) at the story level via `StoryPublicApi::isAuthor`. Beta readers are not blocked.
