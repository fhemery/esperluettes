...
**Secret Gift shuffle is destructive on re-run.** `ShuffleService::performShuffle()` deletes all existing assignment **rows** before creating new ones — the uploaded files are not touched. Gift images survive as unclaimed paths and `media:gc` reclaims them; gift sounds are orphaned on the `local` disk with nothing to collect them. The Artisan command prompts for confirmation and the admin panel button sits behind a confirm modal, but a programmatic caller of `ShuffleService` directly has no guard.

**Secret Gift enrolment is gated by one predicate, everywhere.** `SecretGiftConfigService::isRegistrationOpen()` — activity in `preview`, before the settings' `registration_ends_at`, not yet shuffled — is re-derived by the reader page *and* by every write endpoint (`SecretGiftParticipantController::store()`/`update()`/`destroy()`), never trusted from a rendered page. A Secret Gift activity with **no** `calendar_secret_gift_settings` row reads as closed, not open — the deadline is mandatory on the admin form, but `CalendarPublicApi::create()` does not run `configRules()`/`persistConfig()`, so anything creating an activity that way (seeders, direct API calls) must write the settings row itself or enrolment silently stays closed.

**Secret Gift `role_restrictions` is enforced by the controller, not by route middleware.** The enrolment write routes carry only `web, auth, verified`; `ActivityService::findVisibleBySlugOrFail()` is what normally checks `role_restrictions`, and these routes never call it. `SecretGiftParticipantController::assertMayEnrol()` re-checks with `AuthPublicApi::hasAnyRole()` and 404s. A new Secret Gift write route must repeat this check, not assume the middleware covers it.

**Secret Gift shuffle role gate uses the project's standard `role:` middleware**, which redirects to the dashboard with an error rather than returning a 403. Do not write a test or a check expecting a 403 on `calendar.admin.secret-gift.shuffle` — the request never reaches `ShuffleService` for a disallowed role, but the HTTP response is a redirect.
...
- `Auth::UserDeleted` → SecretGift `RemoveParticipantOnUserRemoved::handle()` — un-enrols the user from every Secret Gift not yet shuffled.
- `Auth::UserDeactivated` → SecretGift `RemoveParticipantOnUserRemoved::handle()` — same handler, same rule; deletion and deactivation are treated identically here.

Subscriptions are wired in `JardinoServiceProvider::registerEventListeners()`, `QuoteContestServiceProvider::registerEventListeners()` and `SecretGiftServiceProvider::registerEventListeners()` via `EventBus`.
...
**A type may carry its own configuration.** Beyond `displayComponentKey()`, a registration can return a `configComponentKey()` — a Blade component rendered inside the admin activity create/edit form — and back it with `configRules()` (validation rules merged into `ActivityRequest` for that type only) and `persistConfig(int $activityId, array $validated)`. `ActivityController::store()`/`update()` run the activity write and `persistConfig()` in a single `DB::transaction()`, so an activity can never exist without its type config; throwing from `persistConfig()` rolls the activity back. This only runs from the admin form — `CalendarPublicApi::create()` does not call `configRules()`/`persistConfig()`, so a type-specific settings row is not guaranteed for activities created through the Public API. Jardino declares none of the three; Secret Gift and Quote Contest both do, and share `Private/Support/DateOrderRule.php` for cross-field date validation within the same request payload. On create the type is chosen in the same form, so every declared config panel is rendered and toggled client-side on the `activity_type` select — only the submitted type's rules are applied server-side. A panel needing its own `<form>` (Quote Contest's category editor, Secret Gift's shuffle button) must push it to the `activity-config-extras` stack, which the create/edit pages render after `</form>`: nested forms are illegal HTML and browsers silently drop the inner one.
