<x-admin::layout>
    <div class="flex flex-col gap-6">
        <x-shared::title>{{ __('config::admin.feature_toggles.create_title') }}</x-shared::title>

        <x-shared::flash-block />

        <form method="POST" action="{{ route('config.admin.feature-toggles.store') }}" class="flex flex-col gap-6">
            @csrf

            <div>
                <x-shared::input-label for="name" :required="true">{{ __('config::admin.feature_toggles.form.name') }}</x-shared::input-label>
                <x-shared::text-input id="name" name="name" :value="old('name', '')" class="font-mono" />
                <x-shared::input-error :messages="$errors->get('name')" />
            </div>

            <div>
                <x-shared::input-label for="domain" :required="true">{{ __('config::admin.feature_toggles.form.domain') }}</x-shared::input-label>
                <x-shared::text-input id="domain" name="domain" :value="old('domain', 'config')" class="font-mono" />
                <x-shared::input-error :messages="$errors->get('domain')" />
            </div>
            ...
            <div>
                ...
                <select id="admin_visibility" name="admin_visibility"
                    ...
                    <option value="all_admins" {{ old('admin_visibility') === 'all_admins' ? 'selected' : '' }}>
            ...
            <div>
                ...
                <div x-data="{ currentValue: @js(old('roles', [])), saving: false }" class="mt-1">
