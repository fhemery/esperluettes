...
it('shows the Export CSV button on the Feedbacks list page', function () {
    ...
    assert_export_button($this, '/admin/story/feedbacks');
});

it('streams a valid CSV with expected headers and data', function () {
    $f1 = StoryRefFeedback::create([
        'name' => 'Grammar',
        'slug' => 'grammar',
        'description' => 'Language/grammar issues',
        'is_active' => true,
        'order' => 1,
    ]);

    $f2 = StoryRefFeedback::create([
        'name' => 'Pacing',
        'slug' => 'pacing',
        'description' => 'Story pacing feedback',
        'is_active' => false,
        'order' => 2,
    ]);
    ...
    $stream = csv_stream_from_list_page(ListFeedbacks::class, $columns, 'feedbacks.csv');
    ...
    expect($ids)->toContain($f1->id, $f2->id);
