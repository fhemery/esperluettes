    ...
    public function panel(Panel $panel): Panel
    {
        return $panel
            ...
            ->navigationItems([
                ...
                NavigationItem::make('Codes d\'activation')
                    ->url('/admin/auth/activation-codes')
                    ->icon('heroicon-o-key')
                    ->group('Gestion des utilisateurs')
                    ->sort(3)
                    ->visible(fn (): bool => auth()->user()?->hasRole([Roles::ADMIN, Roles::TECH_ADMIN]) ?? false),
                ...
                NavigationItem::make('Modération des histoires')
                    ->url('/stories/admin/moderation')
                ...
                NavigationItem::make('Raisons de signalement')
                    ->url('/admin/moderation/moderation-reasons')
                    ->icon('heroicon-o-flag')
                    ->group('Modération')
                    ->sort(1)
                    ->visible(fn (): bool => auth()->user()?->hasRole([Roles::ADMIN, Roles::TECH_ADMIN, Roles::MODERATOR]) ?? false),
