...
5. Append a row to the backlog table: title, folder, proposed mode
   (`auto` for a bugfix or chore, `interactive` otherwise), status `TODO`.
6. Report the row and where it landed in the order. Suggest a different position
   if the task blocks or is blocked by another row.
