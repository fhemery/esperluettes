 ...
 *
 * `pnpm install` brings the playwright package, but not the browser binary,
...
// 1. The pnpm package — a devDependency, so this is just a guard against a
//    stale node_modules.
let version;
try {
  ...
  fail('playwright is missing from node_modules. Run: pnpm install');
  process.exit(1);
}
...
//    libraries surface; the error names the .so, and `pnpm exec playwright
...
try {
  ...
  console.error('  Try: pnpm exec playwright install-deps chromium');
  process.exit(1);
}
