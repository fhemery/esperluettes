...
- **ADRs under `docs/adr/` must never reference `docs/Feature_Planning`**
  either (active folders or `_done/`). An ADR is durable; fold context into the
  ADR itself. Use the `write-adr` skill. `pnpm run gate` enforces it.
