...
The `pending` action fetches `discord_pending_notifications` that have at least one recipient with `sent_at IS NULL`, ordered by `created_at`. It then calls `NotificationPublicApi::getNotificationsByIds($notificationIds)` to batch-load content, returns each notification's stored payload verbatim as `data`, and derives `defaultText` from `htmlDisplay`.
...
The Discord domain is deliberately type-agnostic: it does not know, derive, rename or supplement any payload key. Whatever a notification type stored via `toData()` is exactly what the bot receives, so a bot that understands a given `type` can rebuild any presentation it wants. The cast to object keeps an empty payload serialising as `{}` rather than `[]`.

> **Rejected design (2026-07-25).** An earlier version of this document specified adding `getUrl()`, `getActorName()` and `getTargetDescription()` to `NotificationContent`, to be implemented by every content class in every domain. Those methods were never implemented, but the controller shipped reading `data['url']`, `data['actor']` and `data['target']` — keys no `toData()` has ever produced — so all three were permanently `null` in production. The passthrough above replaces that design: it needs no interface change, no per-domain work, and cannot silently degrade.

Because payload keys are now part of the bot API contract, `docs/notification-types.md` (auto-generated by `php artisan notifications:export-types-doc`) lists the stored fields of every type and must be regenerated when a type's `toData()` changes.
...
7. ~~Add `getUrl()`, `getActorName()`, `getTargetDescription()` to the `NotificationContent` interface~~ — **cancelled**, see "Notification Content for Discord". The interface is unchanged; `data` is the stored payload passed through verbatim.
8. ~~Implement these three new methods in all existing `NotificationContent` classes~~ — **cancelled**, no per-domain work required.
...
14. Bot API: `pending` returns notifications grouped with recipients list; `markSent` with no `failedRecipients` marks all recipients delivered; `markSent` with `failedRecipients` leaves those recipients pending; authentication required
14b. Bot API payload contract: `data` is the stored payload verbatim — every key present with its JSON type preserved (int stays int, bool stays bool), nullable fields present as `null` rather than omitted, no key added that the type did not define, and `{}` for an empty payload. Tests must use a fixture whose payload keys differ from its rendered text, otherwise a derived payload passes by coincidence.
