...
| --------------------------------------------------------------- | ------------------------------- | ----------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Quotes — in-chapter author view (vNext)                         | `quotes-author-view/`           | interactive | TODO:resume at **BUILD** — REFINE/DESIGN/PLAN are done. `chapters-multi-edit/` (which blocked it, decision #21) is wrapped, so re-read `02-architecture.md` §4 and risk 2 against the new DOM shape: an advanced chapter renders as `div.ce-block.ce-block--text` wrappers inside the single `[data-quote-article]` root. |
| Chapter annotations                                             | `annotations/`                  | interactive | TODO:enters at **BUILD** — `01`/`02`/`03` are the pre-loop documents, 10 of 14 phases remain. Overlaps `chapters-multi-edit/` and `quotes-author-view/` on per-block anchoring — read `chapters-multi-edit/README.md` first and sequence them deliberately.                                     |
