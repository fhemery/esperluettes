...
- **News — comment form retains text after submit** · `news-comment-form-retains-text/` · auto · WIP:BUILD (1/1)
- **Favicon — stays the same when season changes** · `favicon-season-cache/` · auto · TODO: cache-bust favicon URL (e.g. `?theme=<season>`) so season rotation is visible
