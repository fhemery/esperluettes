...
`StaticPage`, `Profile` · **Spec:** [functional](./01-functional.md) ·
...
Deliberate non-goals (functional spec §8):
...
Verification: the visual QA checklist in [`03-plan.md`](./03-plan.md) was run
manually by the maintainer rather than by the `visual-verifier` agent, so the
`OK?` column is left unfilled. The driver script written for it was discarded.
