...
- **The preferences UI hint.** A user who opts in without a linked account gets
  nothing and is told nothing. Needs `DiscordPublicApi::isLinked()` — the
  Discord domain currently has **no public API at all** — and a warning in the
  Discord column of the preferences page.
  → [`../discord-link-hint/`](../discord-link-hint/)
