...
| 11 | 2026-07-31 | VERIFY | Defect D4 — `blocks.*.file` prints the raw key `validation.image`, and its `max:2048` has no message either, because the app publishes no `lang/*/validation.php`. `NewsRequest` has the identical gap. Fix it here, or push it out? | **Backlog row, not fixed here.** The gap is pre-existing and app-wide, not introduced by this feature. Overriding `messages()` in `ChapterRequest` alone would duplicate strings a real fix centralises and would leave News printing raw keys; publishing `lang/fr/validation.php` is a scope widening well past this task. New backlog row: `validation-messages/`. | — |
| 12 | 2026-07-31 | WRAP | What happens to the VERIFY e2e specs in `e2e/tests/features/` (`chapters-multi-edit.spec.ts`, `chapters-multi-edit-reading.spec.ts`)? | **Promote a trimmed block-editor case to `e2e/tests/core/multi-editor.spec.ts`** (add / insert + reorder / delete with the state resync) and retire everything else. `<x-editor::multi>` is shared by News, chapters and soon static pages, and defect D2 was a browser-only breakage of exactly that component that no server-side test could catch. The reading-side spec is retired outright: its claims are either PHP-testable or one-off typography measurements. | — |
