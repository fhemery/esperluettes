...
| 7 | Consequence of #6 measured in BUILD: on `/stories` with hide-display on, the *reference* trigger-warning names (e.g. « Violence ») are still present in the page source — the `exclude_tw` multi-select serialises the whole catalog into its Alpine `x-data`. No name is attached to a story, but a reader who wants the words gone entirely still sees them in the filter panel. Reversing #6 would also fix this | BUILD | Yes |
