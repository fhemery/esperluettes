...
⚠️ **`calendar` was declared but wrong.** Calendar's real folder on disk was
`activities/`. Fixed by [`media-consumer-migration`](./media-consumer-migration.md),
which added a proper `activities` scope.
...
- **Three consumers still called `Shared\Services\ImageService` directly**:
  `Calendar\ActivityController`, `StaticPage\StaticPageService`,
  `Profile\ProfileService`. → done, see
  [`media-consumer-migration`](./media-consumer-migration.md).
- **Advanced mode for static pages** — shipped; compact record in
  [`multiedit-static-pages`](./multiedit-static-pages.md).
- **Advanced mode for chapters** — shipped; compact record in
  [`chapters-multi-edit`](./chapters-multi-edit.md).
