...
Two types ship: **Jardino** (a word-count challenge with a flower garden) and
**SecretGift**.
...
| Activity types | `Private/Activities/{Jardino,SecretGift}/` — each with its own provider, migrations, routes |
| Table | `activities` |
...
at `SecretGiftRegistration` for the smaller of the two examples. Your provider
registers the type into `CalendarRegistry`.
...
- **Subscription and participant limits.** `requires_subscription` and
  ...
  list. Both existing types use implicit participation.
...
(`Private/Controllers/Admin/`). Media handling is likewise out of date: the
controller still calls `Shared\Services\ImageService` directly and the declared
Media scope `calendar` does not match the real folder `activities/`
