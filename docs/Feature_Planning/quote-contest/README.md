...
> WRAP output — the compact record of the finished feature. **The only file in
> this folder to load by default.** `01`–`03` remain as history.

**Status:** DONE — 2026-08-03 · **Domain(s):** `Calendar` (+ read-only additions
to `Quote`, `Story`) · **Spec:** [functional](./01-functional.md) ·
...
- **Deptrac** — two new edges, `CalendarPrivate → QuotePublic` and `→ NotificationPublic`, as `02-architecture.md` §5 specified.
...
- **#23** (refines #7, taken at VERIFY) — a **deactivated** submitter keeps their name and profile link in *Résultats*; only a **deleted** account shows *Compte supprimé*. `01-functional.md` §5 was amended to match the code, not the reverse.
...
Full list, including 41 assumptions made without asking, in `DECISIONS.md`.

## Not done

**Deliberate non-goals** (§8): no reader-visible results screen ever, no winner
...
**Known drift / accepted nits** (from VERIFY, `03-plan.md` → *VERIFY notes*):
...
**deleted** per `e2e/tests/features/README.md`; nothing else imported them. The
**seeding stays committed**: `E2eCalendarSeeder` (five contests, one per phase —
a phase comes from the clock, so it cannot be fast-forwarded), the deactivated
account in `E2eAccountsSeeder`, the extra stories/quotes, and the `CONTESTS` /
`CONTEST` fixture blocks. They have no consumer right now, on purpose:
`calendar-notifications/` and `calendar-subscription/` will want exactly this
world, and seeding runs once at global setup.

**Pushed back to the backlog:** [`shared-a11y/`](../shared-a11y/00-request.md) —
