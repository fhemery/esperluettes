...
**Domain(s):** `Shared`, `Story` · **Spec:** [request](./00-request.md) ·
...
**Deliberate non-goals** (spec §8): moving the cover tab onto
...
cover custom tab by hand. The two-row visual QA checklist in
[`03-plan.md`](./03-plan.md) is therefore **unfilled**: the custom tab's drop
prompt / max-size hint / oversize error, and SecretGift's sound upload being
unchanged. Note the custom cover tab is behind the `story.custom_covers_enabled`
feature toggle (off by default), so the toggle must be on to see it.
...
**Superseded:** [`shared-image-upload-cleanup/README.md`](../shared-image-upload-cleanup/README.md)
