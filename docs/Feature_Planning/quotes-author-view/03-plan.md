...
`chapitre-illustre-7`. Screenshots in [`shots/`](./shots/).
...
| Surface | Check | OK? |
|---------|-------|-----|
| Author — chapter header | The « n citations » badge sits in the metric row beside reads and word count, indistinguishable from its `metric-badge` neighbours in size, colour and baseline, and reads the same number as the database | ✅ `shots/badge-metric-row.png` — same size, neutral colour and baseline as reads/words; shows the seeded 5 |
...
| Author — heat legibility | Depths 1, 2 and 3+ are distinguishable from one another and from the surrounding prose, and the text stays readable | ✅ `shots/heat-light.png` — three distinct `bg-accent/*` classes on screen at once; depth 1 is faint but visible |
| Author — heat in dark mode | Same, in dark mode | ✅ `shots/heat-dark.png` |
| Author — heat on formatted prose | *(row rewritten — decisions #22/#23)* A passage spanning italics/bold is tinted continuously, **and** a selection spanning two blocks is refused at capture with an inline error | ✅ `shots/heat-formatted-prose.png` — unbroken tint across `<em>` and `<strong>`; `shots/multi-block-refused.png` — « La sélection doit rester dans un seul paragraphe. », save disabled, for two `<p>` and for two `ce-block`s |
| Author — margin markers `md+` | Markers are vertically aligned with the line of their passage, do not drift after fonts and images load, and reflow correctly on window resize | ✅ tested on `chapitre-illustre-7` with the image request held back: the passage moves 480 px on `load` and its marker follows to within 12 px, and again after a viewport change — `shots/markers-after-image-load.png` |
| Author — mobile (`< md`) | No margin marker at all; the tint alone is tappable and opens the popover, anchored below the passage | ✅ `shots/mobile-tint-popover.png` — zero marker elements at 375 px |
| Author — passage popover | Shows the count, then avatar, display name and relative date per reader, newest first; **no note anywhere**; names link to the reader's profile page | ✅ `shots/passage-popover.png` — heading, avatar, name linking to `/profile/e2e-confirmed`, « avant-hier »; the seeded notes appear nowhere |
...
| Author — chapter summary | Opens from the badge with the heat off; one row per passage with its count, ordered by count descending | ✅ `shots/summary-popup.png` — 4 rows, count-2 passage first, heat still off |
...
| Author — keyboard | Badge, toggle, summary rows, tints and markers are reachable by Tab and activate with Enter/Space; each announces its count | ❌ **the badge is not keyboard reachable.** Toggle, tint and marker are focusable, announce their count and activate with Enter. The badge trigger is `<x-shared::popover>`'s `<div role="button">` with no `tabindex`, so the chapter summary — the only place a stale passage is explained — cannot be opened without a mouse. Pinned by a `test.fail()` spec |
