...
| 12 | 2026-08-05 | WRAP | Plan open item #3 — a reply is still possible on an article unpublished after the thread existed, because `CommentPublicApi::create()` never calls `canReply()`. Leave as a documented non-issue, or fix? | Fix it now, in News: `NewsCommentPolicy::validateCreate()` throws `body: Comment not allowed` when the article is not published (it is the only policy hook Comment runs on both the root and the reply path). `canReply()` stays `true` — it is called once per rendered comment and a lookup there would be one query per comment. Three cases added to `NewsCommentPolicyTest`. | — |
