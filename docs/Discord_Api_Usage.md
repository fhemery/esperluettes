...
- `data` (array): Array of notification objects — one entry per notification, regardless of recipient count
  ...
  - `sourceUserId` (integer|null): Website user ID of the user whose action triggered the notification. **`null` means the notification was generated by the system** rather than by a user (e.g. a scheduled publication). Always present; a sibling of `data`, never nested inside it.
  - `avatarUrl` (string|null): Avatar URL of the user identified by `sourceUserId`; `null` when `sourceUserId` is `null`
